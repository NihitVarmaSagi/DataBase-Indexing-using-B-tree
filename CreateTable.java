...
public class CreateTable {
	/* Below method splits the create command */
	...
	/* below method parses tokens to
	 extract PRIMARY, NOT NULL conditions */
		...
		try {
			...
			for (int h=0; h<strArray.length;h++)   {
				...
				columns.add(tableName);
															//table_name column_name column_key data_type ordinal_position is_nullable 
				
					columns.add(tokens[0]);
					if(h==1) {								//first column is INT PRIMARY KEY
						columns.add("PRI");
			...
			newTable.setLength(512);                            //creates a new file with table name and initializes data
			newTable.seek(0);

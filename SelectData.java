...
public class SelectData {
	...
	public static void selectall(String userCommand) {
		...
		try {
         ...
         if (fileTemp.exists()){
			...
			while(no_of_pages!=0){
				byte z1 = tableFile.readByte(); // identifier
				if(z1==13){
				byte z2 = tableFile.readByte(); // number of rows
				int z3 = tableFile.readShort(); // start area or last record
				int[] z4 = new int[z2];
				LinkedList<Integer> l1=new LinkedList<Integer>();
				for (int i = 0; i < z2; i++) {
					z4[i] = tableFile.readShort(); // all rows address
				}
				...
				for (int j = 0; j < l1.size(); j++) {
					...
					tableFile.seek(l1.get(j));
						int x1 = tableFile.readShort(); // payload
						int x2 = tableFile.readInt(); // row id
						insideList.add(Integer.toString(x2));
						int x3 = tableFile.readByte(); // additional columns
						int[] a = new int[x3];
						int[] c = new int[x3];
						for (int i = 0; i < x3; i++) {
						...
						for (int i = 0; i < x3; i++) {
							...
							if(c[i]==0){
								...
								{
									// insideList.add(Byte.toString(tableFile.readByte()));
									 insideList.add("NULL");
									// insideList.add(Integer.toString((int)tableFile.readByte()));
								}
								if(a[i]==4)
								{
									//System.out.println(Integer.toString((int)tableFile.readByte()));
									insideList.add(Integer.toString((int)tableFile.readByte()));
								}
								if(a[i]==5)
								{
								...
								if(a[i]==11)
								{
									...
									//insideList.add(Long.toString(tableFile.readLong()));
									String str = "";
									for (int l = 0; l < 10; l++)
										str += (char) tableFile.readByte();
									insideList.add(str);		
	...
	public static ArrayList<ArrayList<String>> getColumn(String tableName) {
	   ...
	   while(no_of_pages!=0){
			...
			byte z1 = columnsTableFile.readByte(); // identifier
			if(z1==13){
			int z2 = columnsTableFile.readByte(); // number of rows
			int z3 = columnsTableFile.readShort(); // start area
			...
			do {
				...
				if (columnsTableFile.getFilePointer() < columnsTableFile.length()) {
					int x1 = columnsTableFile.readShort(); // payload
					int x2 = columnsTableFile.readInt(); // row id
					int x3 = columnsTableFile.readByte(); // additional columns
															// other than
															// primary key
					if (item == -18) {
						item = z2;
					}
					int[] a = new int[x3 + 1]; // Size of total columns(6+1)
					int[] c = new int[x3 + 1]; // size of total columns(6+1)
					String[] store = new String[x3 - 2];
					for (int i = 1; i <= x3; i++) {
						int b = columnsTableFile.readByte();
						if (b > 12) {
							a[i] = b - 12; // a[i],c[i] has type and length
							c[i] = 1;
						} else {
							a[i] = b;
							c[i] = 0;
					...
					for (int k = 1; k < 3; k++) {
						...
						if (tableName.equalsIgnoreCase(str1)) {
							// Get required table and set flag1 to identify
							// later.
							flag1 = 1;
						}
						if (k == 2 && flag1 == 1) {
							// we will get column name for k=1 iteration
							columnName = str1;
						}
					}

					for (int r = 3; r <= x3; r++) { // for all columns starting
													// after columnname
						...
						if (c[r] == 1) {
							...
							// This is loop is parse through entire column tables
							if (a[r] == 5) {

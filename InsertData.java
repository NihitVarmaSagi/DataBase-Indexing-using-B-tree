...
public class InsertData {
	...
	/* Below method parses given
	 * insert command,retrieves required data and
	...
	public static void insert(String command){              
	   try {
			...
			try {
				...
				col = arrayList.get(0);                   				//gets column_key,data_type, is_nullable values from 
				type = arrayList.get(1);								//davisbase_columns
		...
		if(CheckPK(pk,table_file) == 1) {										//if primary key condition is not satisfied quit
			   ...
			   if(space>=newrow_space) {										// if space is available insert
		           ...
		           int old_start;
		        	...
		        	if(no_of_pages==1) {		        		 
		        		 ...
		        		 tableFile.setLength(len+page_size);						//create interior page
	...
	/*
	 * Checks if give row_id is unique
	 */
	...
	/*
	 * Below method is used to
	 * calculate left over space in page
	 */
	...
	/*
	 * Below method is used to caluclate payload of
	 * row to be inserted
	 */
	...
	/* Below method checks if NOT NULL constraints are
	 * satisfied
	 */
	private static int[] add_data(String[] values, String[] column_type, String[] cons ) {
	 ...
	 /*
	  * Below method is used to get serial codes of each column
	  */
